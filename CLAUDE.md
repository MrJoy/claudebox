...
There is no linter. Syntax-check the shell without Docker via `bash -n entrypoint.sh` / `bash -n claudebox.sh`, and run the tests with `./test-providers.sh` (optionally `./test-providers.sh <substring>` to filter by case label) and `./test-shim.sh`.
